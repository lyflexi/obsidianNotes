# 提前导入sql脚本
服务器上找一个MySQL8数据库导入，后续Nacos配置文件连接数据库实现持久化
```sql
-- --------------------------------------------------------  
-- 主机:                           192.168.18.100  
...
# 准备好挂载目录
```shell
mkdir -p /root/datamapping/nacos
vi /root/datamapping/nacos/custom.env
```
配置文件custom.env内容如下：
有一个MYSQL_SERVICE_HOST也就是mysql地址，需要修改为你自己的虚拟机IP地址，包括下面的端口号用户名密码一并修改
```shell
PREFER_HOST_MODE=hostname  
MODE=standalone  
SPRING_DATASOURCE_PLATFORM=mysql  
MYSQL_SERVICE_HOST=192.168.18.100  
...
```
开始安装，注意在/root目录下执行安装命令，--env-file只识别相对路径./datamapping/nacos/custom.env
```PowerShell
docker run -d \
--name nacos \
--env-file ./datamapping/nacos/custom.env \
-p 8848:8848 \
-p 9848:9848 \
-p 9849:9849 \
--restart=always \
...
启动完成后，访问下面地址：http://192.168.18.100:8848/nacos/#/login，

首次访问会跳转到登录页，账号密码都是nacos
![[Pasted image 20240125153441.png]]
