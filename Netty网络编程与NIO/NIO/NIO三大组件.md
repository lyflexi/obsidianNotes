...
channel 有一点类似于 stream，它就是读写数据的==双向交互通道==，可以从 channel 将数据读入 buffer，也可以将 buffer 的数据写入 channel，相比于过去的JavaStream（字节流/字符流/文件流，要么是仅输入要么是仅输出），NIO中的channel是双向的并且更为底层
![[1 1.png]]
常见的 Channel 有
...
## 服务器单线程非阻塞模式设计
目前进化到了单线程非阻塞IO模式
- ssc.configureBlocking(false)，服务器的accept()方法不再阻塞会返回 null继续运行-ServerSocketChannel.accept
...
// 3. 连接集合  
        ...
        while (true) {  
            ...
            for (SocketChannel channel : channels) {  
                // 5. 接收客户端发送的数据  
                int read = channel.read(buffer);// 非阻塞，线程仍然会继续运行，如果没有读到数据，read 返回 0                if (read > 0) {  
                    buffer.flip();  
                    debugRead(buffer);  
                    buffer.clear();  
                    log.debug("after read...{}", channel);  
...
ssc.bind(new InetSocketAddress(8080));
```
ServerSocketChannel初始化成功之后，==单线程进入while循环一开始就要调用selector.select()方法进行事件监听（基于水平触发，当感兴趣的事件到来，该方法会反复被触发，即放行）==
```java
while (true) {  
	//方法1，selector.select()
