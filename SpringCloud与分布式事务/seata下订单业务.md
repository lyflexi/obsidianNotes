...
其中，TM**和**RM可以理解为Seata的客户端部分，引入到参与事务的微服务依赖中即可。将来TM和RM就会协助微服务，实现本地分支事务与TC之间交互，实现事务的提交或回滚。
...
暂时没有解决...
# 分布式事务的究极解决方案
说了这么多，其实分布式事务的究极解决方案就是尽量不要产生分布式事务。因为分布式事务底层涉及网络通信，如果是AT的话还涉及undolog的创建，存储以及执行undolog，十分消耗资源与性能

消息队列通知cart-service和trade-service执行各自的操作，cart-service和trade-service服务接收到消息尝试执行删除购物车以及扣减库存，如果trade-service服务扣减库存失败，人工捕获反复重试，直到成功为止，这就是兜底方案